...
if dlg.OK == False:
    ...
    # left/right response balance
if expInfo['response condition'] == '1':  # blue left and 90° left and black left
    conditionCUE_feet = "A"
    conditionCUE_hand = "A"
    conditionGAB = "A"
    print("blue left and 90° left")
    
elif expInfo['response condition'] == '2':  # yellow left and 0° left and black left
    conditionCUE_feet = "A"
    conditionCUE_hand = "B"
    conditionGAB = "B"
    print("yellow left and 0° left")
    
elif expInfo['response condition'] == '3':  # blue left and 0° left and black left
    conditionCUE_feet = "A"
    conditionCUE_hand = "A"
    conditionGAB = "B"
    print("blue left and 0° left")
    
elif expInfo['response condition'] == '4':  # yellow left and 90° left and black left
    conditionCUE_feet = "A"
    conditionCUE_hand = "B"
    conditionGAB = "A"
    print("yellow left and 90° left")
    
elif expInfo['response condition'] == '5':  # blue left and 90° left and white left
    conditionCUE_feet = "B"
    conditionCUE_hand = "A"
    conditionGAB = "A"
    print("blue left and 90° left")
    
elif expInfo['response condition'] == '6':  # yellow left and 0° left and white left
    conditionCUE_feet = "B"
    conditionCUE_hand = "B"
    conditionGAB = "B"
    print("yellow left and 0° left")
    
elif expInfo['response condition'] == '7':  # blue left and 0° left and white left
    conditionCUE_feet = "B"
    conditionCUE_hand = "A"
    conditionGAB = "B"
    print("blue left and 0° left")
    
elif expInfo['response condition'] == '8':  # yellow left and 90° left and white left
    conditionCUE_feet = "B"
    conditionCUE_hand = "B"
    conditionGAB = "A"
    print("yellow left and 90° left")
...
## create files 
filename = data_dir + os.sep + 'training_'  + '%s_%s' % (expInfo['participant'], expInfo['date']) + '_' + effector_order
filename_short = data_dir + os.sep + expInfo['participant'] + '_' + 'training' + '_' + effector_order
...
# makes 10 first trials cue trial to train new cue SR mapping
for g in range(0,9):
    if g <= 4:
        seq[g] = [1,0]
    else:
        seq[g] = [2,0]
np.random.shuffle(seq[0:9])
...
gabor = visual.GratingStim(win, tex="sin",
                           ...
                           size=stim2_size, sf=[2, 0], ori=0, name='gabor')
...
\t - l'orientation du troisième stimulus qui arrive si le deuxième stimulus est un cercle bruité."
...
delta = 1.2*delta
print(f"threshold loaded. 1.20 * delta = {delta}")
...
while i < len(seq):  # main loop through all trials #
    ...
    if condition == "gabor":
        ...
        kb.clock.reset()
        while True:
            theseKeys = kb.getKeys(keyList=[LH, RH, LF, RF, 'escape'], waitRelease = False)
            if 'escape' in theseKeys:
                core.quit()
            if len(theseKeys):
